...
# Check sources for India events. 
...
def add_value_if_no_or_one_decimal(x, value_to_add):
    if x % 1 == 0 or round(x, 1) == x:  # Check if value has no or only one decimal
        return x + value_to_add
    else:
        return x

# Apply the function to the column
value_to_add = 0.0001  # Define the value to add
...
### Prio shape file ---------
prio_shp = gpd.read_file('/Users/hannahfrank/protest_auto_reg/data/acled/priogrid_cell.shp')
...
prio_help = gpd.read_file("/Users/hannahfrank/protest_auto_reg/data/acled/prio_time_2014.csv")
...
prio_static = gpd.read_file('/Users/hannahfrank/protest_auto_reg/data/acled/prio_static.csv')
prio_static = prio_static.drop(columns=['row','col',"xcoord","ycoord","geometry","cmr_max","cmr_min","cmr_sd","rainseas","maincrop","growend","growstart","imr_max","imr_min","imr_sd","ttime_max","ttime_min","ttime_sd",])
prio_static.replace([''], np.nan, inplace=True)

### Variables in prio grid ###
...
### Save -------
df.isnull().any()
df.to_csv("/Users/hannahfrank/protest_auto_reg/data/acled/acled_grid_India_2023.csv",index=False, sep=',')
...
prio_shp = gpd.read_file('/Users/hannahfrank/protest_auto_reg/data/acled/priogrid_cell.shp')
prio_shp.reset_index(inplace=True, drop=True)
india = df.merge(prio_shp[["gid", "geometry"]],left_on='gid',right_on='gid',how="left")
...
for i in ['2016-01','2016-02','2016-03','2016-04','2016-05','2016-06','2016-07','2016-08','2016-09','2016-10','2016-11','2016-12','2017-01','2017-02','2017-03','2017-04','2017-05','2017-06','2017-07','2017-08','2017-09','2017-10','2017-11','2017-12','2018-01','2018-02','2018-03','2018-04','2018-05','2018-06','2018-07','2018-08','2018-09','2018-10','2018-11','2018-12','2019-01','2019-02','2019-03','2019-04','2019-05','2019-06','2019-07','2019-08','2019-09','2019-10','2019-11','2019-12','2020-01','2020-02','2020-03','2020-04','2020-05','2020-06','2020-07','2020-08','2020-09','2020-10','2020-11','2020-12','2021-01','2021-02','2021-03','2021-04','2021-05','2021-06','2021-07','2021-08','2021-09','2021-10','2021-11','2021-12','2022-01','2022-02','2022-03','2022-04','2022-05','2022-06','2022-07','2022-08','2022-09','2022-10','2022-11','2022-12','2023-01','2023-02','2023-03','2023-04','2023-05','2023-06','2023-07','2023-08','2023-09','2023-10','2023-11','2023-12']:
    ...
    plt.axis('off')
    india.loc[(india["dd"]==i)].plot(column='n_protest_events',ax=ax,cmap="Blues",edgecolor='gray',linewidth=0.3)
    cmap = plt.cm.get_cmap('Blues')
    plt.title(f"Number of protest events in {i}")
    norm = mpl.colors.Normalize(vmin=0, vmax=india.loc[(india["dd"]==i)].n_protest_events.max())
